---
title: Numerically Stable Power Transform
description: "Parametric methods that normalize data but require careful handling to avoid numerical instability."
date: April 14, 2025
bibliography: reference.bib
categories: [Numerical, Transform, Python]
citation:
  id: power-transform
format:
  html:
...
```{python}
#| label: fig-bc-llf-naive
#| fig-cap: "Naive log-likelihood computation."
#| fig-align: center
...
```{python}
#| label: fig-bc-llf-MASS
#| fig-cap: "Log-likelihood after mean division."
#| fig-align: center
...
To address numerical instability for both transformations, log-domain computation [@Haberland2023] is effective. It uses the [Log-Sum-Exp trick](https://en.wikipedia.org/wiki/LogSumExp) to compute statistics in log domain:
...
This allows direct computation of $\ln\sigma^2_\psi$ from $\ln x$. Plugging in @eq-boxcox-log-var, we can compute the log-likelihood in the log-domain.
...
```{python}
#| label: fig-bc-llf-logdomain
#| fig-cap: "Log-likelihood using log-domain computation."
#| fig-align: center
...
Even with log-domain computation, applying the transformation using a large absolute value of $\lambda$ can result in overflow:
