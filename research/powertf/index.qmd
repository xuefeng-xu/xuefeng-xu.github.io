...
Variance terms are also prone to overflow. We construct adversarial datasets that trigger overflow across floating-point precisions for Box-Cox in @tbl-advdata.
...
A naive one-pass method has clients send $(n, \sum x_i, \sum x_i^2)$, and the server sums each component. The variance is then computed as:

$$
\frac{1}{n} \sum_{i=1}^n x_i^2 - \frac{1}{n^2} \left(\sum_{i=1}^n x_i\right)^2
$$ {#eq-var-naive}

[The @eq-var-naive is unstable](/blog/variance.qmd#numerical-cancellation). Instead, we adopt the pairwise algorithm: clients send $(n, \bar{x}, s)$ with mean $\bar{x}$ and sum of squared deviations $s=\sum(x-\bar{x})^2$. The server merges them in a tree structure (@fig-tree-queue):
